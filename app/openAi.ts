...
- 전체 예시 : "훈연을 하면 무슨 맛이 나야되는지 정확히 알고 있거덩요", 
"이게 어떻게 보면 한국 에서만 먹을 수 있고 한국 길거리에서 흔히 접 할 수 있을정도로 대중화된 음식이기 때문에 맛의 기준점 결코 낮지 않은 음식이란 말이죠. '한국의 정'... 그런 맛이 있다고 생각해요 그게 가장 코어가 되고 제가 마지막에 부추 한잎을 곰곰히 씹었는데 부추의 익힘을 저는 굉장히 중요시 여기거든요.
...
- 예시 :  , 
"이 [재료명]는, 어... 제 기준에는 잘못 구워졌어요. [재료명]가 이븐하게 익지 않았어요. [재료명]가 고루 익지 않았어요. 제 생각에는 레스팅을 조금 더 하셨어요 됐고, 그리고 열 전달이 끝까지 잘 안됐어요. 제 생각에는 본인이 알고 있는 지식은 조금 모자란 거 같아요. 그래서 참가자 님은 여기까지인 거 같습니다. 음식은 무궁무진해요. 조금 더 생각을 여세요.
`;
